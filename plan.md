...
5. **Packer.** ✓ — `backend/packer.py`: pure bundle-greedy core (`pack`) over add-on units (exp bullet / open-project bundle / project bullet), density-ranked with the relative `closeness_threshold` breadth tie-break (new project, then least-developed item), honoring pins (waive min-open, override caps), excludes (hard veto), `min_bullets_per_open_project`, and `max_bullets_per_item`; drops zero-relevance content by default. `selection_to_context` maps to the template (fixed bullet first, authored order verbatim). `pack_and_verify` wires it to a real Tectonic compile with a lowest-density trim loop. `tests/test_packer.py` (15 cases, stub heights) passes; `scripts/verify_step5.py` packs the seed library against a sample JD and compile-verifies one page.
6. **FastAPI routes.** ✓ — `backend/app.py`: `GET /api/content`, `POST /api/score` (baseline+LLM blend), `POST /api/pack` (score+pack+real compile), `GET /api/pdf`, `POST /api/export`, plus `/api/health` and `/api/reload`; cached content, single session build dir, static-mounts `frontend/` when present. `backend/archive.py` writes `archive/<date>-<company-slug>/` with `resume.pdf`/`.tex`/`jd.txt`/`selection.json` (re-renders the .tex deterministically). `tests/test_api.py` (TestClient; compile routes skip without Tectonic) + `tests/test_immutability.py` (rendered .tex contains every authored bullet verbatim) pass; `scripts/verify_step6.py` drives score→pack→pdf→export end-to-end against the live LLM.
7. **Frontend.** ✓ — `frontend/` single page (vanilla JS + `fetch`, no framework), served at `/` via the existing `StaticFiles` mount: JD pane + Score/Auto-pack/Export controls; left **Library** grouped by job/project with locked-content summary, fixed-bullet markers, tier badges, tags, per-bullet blended score + matched requirements, grep-style search, and pin/exclude toggles (project-level open/drop too) that live re-pack; center **Proposal** rendering the packed selection in resume order; right **Preview** iframe of `/api/pdf` with a cache-bust per compile and a fit gauge (cm-to-spare / overflow). Bullet text rendered via `textContent` (raw LaTeX, never HTML). `scripts/verify_step7.py` asserts assets present, mounted at `/`, and wired to every API route.
8. End-to-end + tuning. *(next)*
